...
class GridWorld(dm_env.Environment):
    ...
    def observation_spec(self):
        ...
        dict_ = {
            "grid": specs.Array(
                shape=(self.x_dim, self.y_dim, 3), dtype=np.float32, name="grid"
            ),
    ...
    def step(self, action):
                ...
                if self.is_traversable(self.agent_pos, fwd_pos):
                    ...
                    if fwd_cell != None and fwd_cell.type == "goal":
                        ...
                        return dm_env.termination(
                            reward=1.0, observation=self._observation()
                        )
        ...
        if not self._goal:
            if np.sum(np.abs(self.obs[:, :, 1])) < eps:
                ...
                self._reset_next_step = True
                return dm_env.termination(reward=10.0, observation=self._observation())
