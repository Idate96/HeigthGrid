...
class GridObject(metaclass=ABCMeta):
    ...
    def encode(self):
        ...
        """
        return (OBJECT_TO_IDX[self.type], IDX_TO_COLOR[OBJECT_TO_IDX[self.type] % 10], 32)
...
class GridWorld(dm_env.Environment):
    ...
    def __str__(self):
        ...
        repre += "Agent orientation " + np.array2string(DIR_TO_VEC[int(self.agent_dir)]) + "\n"
    ...
    def place_obj(self, obj:GridObject, pos: np.array = np.array([0, 0]), random=False):
    ...
    def remove_obj_at_pos(self, pos:np.array):
        if self.grid_object[pos[0] + self.x_dim * pos[1]] is None:
            warnings.warn("Cannot remove object at (%d, %d), no object present" .format(pos[0], pos[1]))
        else:
    ...
    def is_traversable(self, current_pos, target_pos):
        ...
        heigth_diff = -self.obs[current_pos[0], current_pos[1], 0] + self.obs[target_pos[0], target_pos[1], 0]
    ...
    def can_dig(self, current_pos, target_pos):
        diggable = np.abs((self.obs[current_pos[0], current_pos[1], 0] - self.obs[target_pos[0], target_pos[1], 0])) < 1 + 10**-7
        no_objects = self.get(*target_pos) is None
        return diggable and no_objects
    

    def can_drop(self, current_pos, target_pos):
        height_diff = self.obs[target_pos[0], target_pos[1], 0]  - self.obs[current_pos[0], current_pos[1], 0]
    ...
    def render(self, mode='human', close=False, block=False, key_handler=None, tile_size=SIZE_TILE_PIXELS):
        """
