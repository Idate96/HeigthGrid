...
class ConnectedTrenchEnv(GridWorld):
    def __init__(self, **kwargs):
        self.global_rewards = {"collision_reward": -0.001,  # against wall 0, ok
                               "longitudinal_step_reward": -0.0005,
                               "base_turn_reward": -0.002,  # ok
    ...
    def generate_level(self):
        ...
        occupied_space = 1
        num_blocks = 2
        num_trences = max(1, np.random.randint(self.num_trenches - 1, self.num_trenches + 1))
    ...
    def create_trench(self, map):
        # place randomly the trench but make sure it fits in the grid
        # trenches have value -1 in the map
        # place another trench in the map making sure it overlaps with the previous one
        while True:
            trench_length = self.min_trench_length + np.random.randint(0,
                                                                       self.trench_lenght - self.min_trench_length + 1)

            # pick random trench width between the min and self.trench_width
            trench_width = self.min_trench_width + np.random.randint(0, self.trench_width - self.min_trench_width + 1)
            # if there are not trenches place the first one at random
            if np.sum(map == -1) == 0:
                x = np.random.randint(1, self.map_size - trench_length)
                y = np.random.randint(1, self.map_size - trench_width)
                map[x:x + trench_length, y:y + trench_width] = -1
                map = np.rot90(map, 1)
                break
            # pick random position
            x = np.random.randint(0, self.map_size - trench_length)
            y = np.random.randint(0, self.map_size - trench_width)
            # check if the trench is adjenct to a cell of the previous one
            if np.sum(map[x:x + trench_length, y:y + trench_width] == -1) > 0:
                map[x:x + trench_length, y:y + trench_width] = -1
                break
        return map
...
register(
    id="HeightGrid-ProceduralConnectedTrench-v0",
    entry_point='heightgrid.envs_v2:ConnectedTrenchEnv'
)
